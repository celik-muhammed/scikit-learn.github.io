...
.. tab-set::
  ...
  .. tab-item:: Major/Minor RC
    ...
    - Create a PR from the `main` branch targeting the `1.6.X` branch.
    ...
    - In the `1.6.X` branch, change the version number `__version__` in
    ...
    - Once all the CD jobs have completed successfully in the PR, merge it with the
      ...
      .. warning::
        ...
        a commit with the marker in the `1.6.X` to trigger them.
  ...
  .. tab-item:: Major/Minor Final
    ...
    - Rebase this PR from the `1.6.X` branch:
    ...
    - In the `main` branch, edit the corresponding file in the `doc/whats_new` directory
      ...
      the previous major/minor version is `1.5.1`, then you can use the
      following command to retrieve the list of contributor names:

      .. prompt:: bash

        git shortlog -s 1.5.1.. |
          cut -f2- |
          sort --ignore-case |
          tr "\n" ";" |
          sed "s/;/, /g;s/, $//" |
      ...
      Then cherry-pick it in the `1.6.X` release branch.
  ...
  .. tab-item:: Bug-fix
    ...
    Suppose that we are preparing the release `1.5.2`.

    

    - Create a PR from the `main` branch targeting the `1.5.X` branch.
      Copy the following release checklist to the description of this PR to track the
      progress.
    ...
    - Rebase this PR from the `1.5.X` branch:
    ...
    - In the `1.5.X` branch, change the version number `__version__` in
      `sklearn/__init__.py` to `1.5.2`.

    
    - In the `main` branch, edit the corresponding file in the `doc/whats_new` directory
      to update the release date
      and add the list of contributor names. Suppose that the tag of the last release in
      the previous major/minor version is `1.4.2`, then you can use the
      following command to retrieve the list of contributor names:
      ...
      Then cherry-pick it in the `1.5.X` release branch.
    ...
    - Once all the CD jobs have completed successfully in the PR, merge it with the
      ...
      .. warning::
        ...
        In addition if on merging, the last commit, containing the `[cd build]` marker,
        is empty, the CD jobs won't be triggered. In this case, you can directly push
        a commit with the marker in the `1.5.X` to trigger them.
    ...
    - If the steps above went fine, proceed **with caution** to create a new tag for the
      ...
      .. prompt:: bash
        ...
        git tag -a 1.5.2  # in the 1.5.X branch
        git push git@github.com:scikit-learn/scikit-learn.git 1.5.2
    ...
    - Update `SECURITY.md` to reflect the latest supported version `1.5.2`.
